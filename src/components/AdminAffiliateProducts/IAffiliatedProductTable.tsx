...
import type { IAffiliatedProduct, AffiliatedProductPayload } from "@/redux/types/IAffiliatedProduct.type";
import {
...
const IAffiliatedProductTable: React.FC = () => {
  ...
  const { register, handleSubmit, setValue, formState: { errors } } = useForm<AffiliatedProductFormType>({
    defaultValues: {
      title: "",
      price: 0,
      avgRating: 0,
          ...
          <thead className="bg-gray-50 border-b border-gray-300">
            <tr>
              {["Title", "Company", "Price", "Image URL", "Link", "Actions"].map((header) => (
                <th key={header} className="px-6 py-3 text-left font-medium text-gray-700">{header}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {isLoading || isFetching ? (
              <tr><td colSpan={6} className="p-6 text-center"><PageLoader /></td></tr>
            ) : isError ? (
              <tr><td colSpan={6} className="p-6 text-center text-red-500">Error fetching products</td></tr>
            ) : products.length === 0 ? (
              ...
              products.map((p) => (
                <tr key={p.id} className="border-b border-gray-300 hover:bg-gray-50 transition duration-200">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    {p.image_url ? <a href={p.image_url} target="_blank" rel="noreferrer" className="text-blue-600 underline truncate block max-w-[150px]">Visit</a> : <span className="text-sm text-gray-500">No Image</span>}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
      ...
      {viewProduct && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="max-w-2xl max-h-[80vh] overflow-y-auto rounded-lg bg-white p-6">
            ...
            <p><strong>Avg Rating:</strong> {viewProduct.avg_rating}</p>
            <p><strong>Total Ratings:</strong> {viewProduct.total_review}</p>
            {viewProduct.image_url && <img src={viewProduct.image_url} alt={viewProduct.title} className="my-3 h-48 w-auto rounded border object-contain" />}
