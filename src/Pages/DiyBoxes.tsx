...
export default function DiyBoxes() {
  ...
  const availableAgeRanges = useMemo(() => {
    ...
    activities.forEach((activity) => {
      if (activity.age_range) ranges.add(activity.age_range);
    });
  ...
  const filteredActivities = useMemo(() => {
    return activities.filter((activity) => {
      ...
      const matchesAge =
        !ageRange ||
        (activity.age_range &&
          activity.age_range.toLowerCase() === ageRange.toLowerCase());
      const matchesTheme =
        !theme ||
        (activity.theme &&
          activity.theme.toLowerCase() === theme.toLowerCase()) ||
        (activity.product_type &&
          activity.product_type.toLowerCase() === theme.toLowerCase());
  ...
  const totalPages = Math.ceil(total / boxesPerPage);
  const paginated = useMemo(() => {
    const start = (page - 1) * boxesPerPage;
    return filteredActivities.slice(start, start + boxesPerPage);
  }, [page, filteredActivities]);

  const pageFromServer = page;
