...
const authSlice = createSlice({
  ...
  reducers: {
    setUser: (state, action: PayloadAction<{ user: User; token: string }>) => {
      ...
      Cookies.set("token", action.payload.token, { expires: 7 }) // 7 days
  ...
  extraReducers: (builder) => {
    builder.addMatcher(authApi.endpoints.login.matchFulfilled, (state, { payload }: PayloadAction<LoginResponse>) => {
      ...
      if (user) {
        ...
        Cookies.set("token", payload.data.accessToken, { expires: 7 })
        if (typeof window !== "undefined") {
    ...
    builder.addMatcher(authApi.endpoints.register.matchFulfilled, (state, { payload }) => {
      console.log("Registration successful:", state,payload.data)
    })
  },
})
