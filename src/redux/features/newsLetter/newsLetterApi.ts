...
export const newsLetterApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    getAllNewsletters: builder.query<Newsletter[], void>({
      query: () => ({ url: "/news-letter", method: "GET" }),
      providesTags: ["NewsLetter"],
      transformResponse: (response: any) => response.data,
    }),

    subscribeNewsletter: builder.mutation<Newsletter, SubscribePayload>({
      query: (body) => ({ url: "/news-letter", method: "POST", body }),
      invalidatesTags: ["NewsLetter"],
    }),

    deleteNewsletter: builder.mutation<{ success: boolean }, string>({
      query: (idOrEmail) => ({ url: `/news-letter/${idOrEmail}`, method: "DELETE" }),
      invalidatesTags: ["NewsLetter"],
    }),

    sendPromotionalMail: builder.mutation<PromotionalMailResponse, PromotionalMailPayload>({
      query: (body) => ({ url: "/news-letter/promotional-mail", method: "POST", body }),
    }),
  }),
});
